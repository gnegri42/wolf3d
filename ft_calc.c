...
static void	ft_find_wall(t_mlx *mlx, t_map *map, t_player *player, t_ray *ray)
{
	...
	{		
		...
		{
			ray->step_x = 1;
			ray->side_dist_x = (player->map_x + 1 - ray->pos_x) * ray->delta_dist_x;
		}
		if (ray->dir_y < 0)
		{
			ray->step_y = -1;
		...
		{
			ray->step_y = 1;
			ray->side_dist_y = (player->map_y + 1 - ray->pos_y) * ray->delta_dist_y;
		}
		...
		if (ray->wall_dist <= 0.05)
			ray->wall_dist = 0.05;
		map->line_height = (int)(WIN_HEIGHT / ray->wall_dist);
		map->pixel_start = (WIN_HEIGHT - map->line_height)/ 2;
		if (map->pixel_start < 0)
			map->pixel_start = 0;
		map->pixel_last = (WIN_HEIGHT + map->line_height) / 2;
